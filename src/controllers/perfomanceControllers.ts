...
const getDaysElapsedInMonth = (year: number, month: number) => {
  const now = new Date();
...
export const getAgentPerformanceTracker = async (req: Request, res: Response) => {
  try {
    const month =
    ...
    const results = await Promise.all(
      members.map(async (member) => {
        const [target, disbursedAgg, pendingAgg, disbursedCount] = await Promise.all([
          prisma.agentMonthlyTarget.findUnique({
            where: {
        ...
        const salesPercentage =
          salesTarget > 0 ? (paidSales / salesTarget) * 100 : 0;
        ...
        const projectedTarget =
          daysElapsed > 0 ? (paidSales / daysElapsed) * daysInMonth : 0;
...
export const getMonthlyGoalTracker = async (req: Request, res: Response) => {
  try {
    const month =
      typeof req.query.month === "string" ? Number(req.query.month) : undefined;
    const year =
...
export const getTargetAchievedRate = async (req: Request, res: Response) => {
  try {
    const month =
      typeof req.query.month === "string" ? Number(req.query.month) : undefined;
    const year =
    ...
    const results = await Promise.all(
      members.map(async (member) => {
        ...
        const achievedRate =
          monthlyTarget > 0 ? (disbursed / monthlyTarget) * 100 : 0;
...
export const getTeamPerformanceSummary = async (req: Request, res: Response) => {
  try {
    ...
    const [teamTarget, disbursedAgg, pendingAgg, rejectedCount, disbursedCount] =
      await Promise.all([
        prisma.teamMonthlyTarget.findUnique({
          where: {
            month_year: {
              month: selectedMonth,
              year: selectedYear,
    ...
    const teamAchievedRate =
      totalTeamTarget > 0 ? (totalDisbursed / totalTeamTarget) * 100 : 0;
